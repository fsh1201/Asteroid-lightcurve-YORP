/*     A procedure for shape+spin+scattering modelling of lightcurves
	   ...
	   Modified by Shuai Feng to take the YORP acceleration into account, 
	   and some of the global variables are changed to local variables that can be passed to functions,
	   and the very big memories are changed to be malloced on heap to avoid segmentation fault.
...
FILE* f_1, * f_lc_out, * f_areas, * f_par, * fm_in, * fm_out, * fm_parin;
...
int main(int argc, char* argv[])
{
	...
	if (argc < 2)
	{
		printf("[cat lc |] ./convexinv [[-s] [-v]] [[-o out_areas] [-p out_par] [input_convexinv] [out_lcs]] [[-io lc_in lc_out] [[par_in] [-np]]] "
			"[-pari cg_in [-cglc lc_in]] [-paro cg_out]\n");
		printf("lc: observed lightcurves\n");
		printf("-s -v: only can be chosen one of them\n");
		printf("out_areas: gaussian surface density\n");
		printf("out_par: spin state and scattering parameters\n");
		printf("input_convexinv: configuration of convexinv\n");
		printf("lc_in: lightcurves which are used for times and coordinates\n");
		printf("lc_out: output synthetic lightcurves\n");
		printf("par_in: spin state and scattering parameters used for simulation\n");
		printf("-np: use the inversed spin state and scattering parameters as par_in\n");
		printf("cg_in: input all parameters in iteration for simulation\n");
		printf("cg_out: output all parameters in iteration\n");
		exit(0);
	}
	int i, j, l, m, k, nrows, ndata, k2, ndir,
		* ia, ial0, ial0_abs, ia_beta_pole, ia_lambda_pole, ia_prd, ia_par[4], ia_cl,
		ind_par_file, ind_lc_out_file, ind_out_file,
	...
	/* initial YORP strength fixed or free */
	fscanf(f_1, "%lf %d", &nu, &ia_nu);							fgets(str_temp, MAX_LINE_LENGTH, f_1);
	/* epoch of zero time */
	...
	fscanf(f_1, "%lf %d", &par[4], &ia_par[4]);                fgets(str_temp, MAX_LINE_LENGTH, f_1);
	...
	trifac(nrows, ifp);	//calculate **ifg
	/* areas and normals of the triangulated Gaussian image sphere */
	areanorm(t, f, ndir, Numfac, ifp, at, af, Nor, Darea);	//calculate **at, **af, *Darea, **Nor
	/* precompute some function values at each normal direction*/
	sphfunc(Numfac, at, af, Mmax, Lmax, Ts, Tc, Fs, Fc, Pleg, Dsph);	//calculate **Fc, **Ts, **Tc, **Fs, ***Pleg, **Dsph
	/* sph. harm. coeffs. of the initial ell. */
	ellfit(cg, a, b, c, Numfac, Ncoef, at, af, Mmax, Lmax, Pleg);	//calculate the initial *cg, the number is Ncoef
	...
	if (onlyrel == 0)
		ia[1] = 1;
	ia[Ncoef + 4 + Nphpar + 1] = ia_cl;
	/* Lommel-Seeliger part is fixed */
	ia[Ncoef + 4 + Nphpar + 2] = 0;

	if ((Ncoef + 4 + Nphpar + 1) > MAX_N_PAR)
	{
	...
	{
		mrqmin(ee, ee0, tim, brightness, sig, cg, ia, Ncoef + 4 + Nphpar + 2, covar, aalpha, bright, Nphpar, Deallocate, Lastcall, Numfac, Mmax, Lmax,
		...
		if ((Niter == 1) || (Chisq < Ochisq))
		{
			...
			curv(cg, Numfac, Mmax, Lmax,
				Fc, Fs, Pleg, Area, Darea, Dsph, Dg);	//calculate *Area, **Dg
			for (i = 1; i <= 3; i++)
			{
				chck[i] = 0;
				totarea = 0;
	...
	if (verbose == 1)
	{
		printf("\nlambda, beta, period (hrs) and YORP strength (rad/day^2): %f %+f %f %g\n", cg[Ncoef + 2] * RAD2DEG, 90 - (cg[Ncoef + 1] * RAD2DEG), 24 * prd, cg[Ncoef + 4]);
		printf("phase function parameters: ");
		for (i = 1; i <= Nphpar; i++)
			printf("%f ", cg[Ncoef + 4 + i]);
	}
	if (verbose == 1)
		printf("\nLambert coefficient: %g\n", exp(cg[Ncoef + Nphpar + 5]));
	Alamda = 0;
	mrqmin(ee, ee0, tim, brightness, sig, cg, ia, Ncoef + 4 + Nphpar + 2, covar, aalpha, bright, Nphpar, Deallocate, Lastcall, Numfac, Mmax, Lmax,
		Lcurves, Inrel, Lpoints,
		&Alamda, &Scale, Blmat, Dblm,
	...
	if (paramout == 1)
	{
		...
		for (i = 1; i <= Nphpar; i++)
			fprintf(f_par, "%.15g ", cg[Ncoef + 4 + i]);
		fprintf(f_par, " %.15g\n", exp(cg[Ncoef + Nphpar + 5]));
